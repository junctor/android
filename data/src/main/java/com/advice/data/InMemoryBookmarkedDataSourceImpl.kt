...
class InMemoryBookmarkedDataSourceImpl : BookmarkedElementDataSource {
    ...
    override suspend fun isBookmarked(content: Content): Boolean = map.containsKey("content:${content.id}")

    override suspend fun isBookmarked(session: Session): Boolean = map.containsKey("session:${session.id}")

    override suspend fun isBookmarked(tag: Tag): Boolean = map.containsKey("tag:${tag.id}")
